...
const Player = () => {
      ...
      >
        ...
        <div className="flex items-center gap-8 mb-12">
          <button onClick={toggleShuffle} className="relative">
            <Shuffle
              size={24}
              className={isSmartShuffle ? "text-blue-400" : "text-zinc-400"}
            />
            {isSmartShuffle && (
              <span className="absolute -top-2 -right-3 text-[10px] bg-blue-400 text-black px-1 rounded-sm font-bold">
                AI
              </span>
            )}
          </button>
  ...
  return (
    <div
      className={`fixed bottom-[64px] md:bottom-0 left-0 right-0 bg-blue-900 md:bg-zinc-900 border-t border-zinc-800 px-4 py-2 md:h-24 flex items-center justify-between z-40 transition-all duration-300 ${
        isMenuOpen
          ? "translate-y-full opacity-0 pointer-events-none"
          : "translate-y-0 opacity-100"
      }`}
      ...
      <div className="hidden md:flex flex-col items-center gap-2 flex-1 max-w-[600px]">
        <div className="flex items-center gap-6">
          <button onClick={toggleShuffle} className="relative group">
            <Shuffle
              size={18}
              className={`transition ${isSmartShuffle ? "text-blue-400" : "text-zinc-500 group-hover:text-white"}`}
            />
            {isSmartShuffle && (
              <span className="absolute -top-2 -right-2 text-[8px] bg-blue-400 text-black px-0.5 rounded-sm font-bold">
                AI
              </span>
            )}
          </button>
          <SkipBack
            size={20}
            className="text-zinc-400 hover:text-white fill-current cursor-pointer"
            onClick={prevTrack}
          />
          <button
            onClick={togglePlay}
        ...
        <div className="w-full flex items-center gap-2">
          ...
          <div className="flex-1 h-1 bg-zinc-700 rounded-full relative group">
            <div
              className="absolute h-full bg-blue-500 group-hover:bg-[#1ed760] rounded-full"
              style={{ width: `${progressPercent}%` }}
            />
          </div>
