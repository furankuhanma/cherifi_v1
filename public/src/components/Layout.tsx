    ...
    <aside className="hidden md:flex flex-col w-64 bg-black p-6 gap-8 h-screen sticky top-0 border-r border-zinc-800">
      ...
      <nav className="flex flex-col gap-4 text-zinc-400 font-medium">
        <NavLink
          to="/"
          className={({ isActive }) =>
            `flex items-center gap-4 hover:text-white transition ${isActive ? "text-white" : ""}`
          }
        >
          <Home size={24} /> Home
        </NavLink>
        ...
        <NavLink
          to="/offline"
          className={({ isActive }) =>
            `flex items-center gap-4 hover:text-white transition relative ${isActive ? "text-white" : ""}`
          }
        >
      ...
      <div className="mt-auto border-t border-zinc-800 pt-4">
        <p className="text-xs text-zinc-500 hover:underline cursor-pointer">
          Cookies
        </p>
        <p className="text-xs text-zinc-500 hover:underline cursor-pointer mt-2">
          Privacy Policy
        </p>
      </div>
    </aside>
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-zinc-900 bg-opacity-95 backdrop-blur-md border-t border-zinc-800 flex justify-around py-3 z-50">
      <NavLink
        to="/"
        className={({ isActive }) =>
          `flex flex-col items-center gap-1 text-[10px] ${isActive ? "text-white" : "text-zinc-400"}`
        }
      >
      ...
      <NavLink
        to="/offline"
        className={({ isActive }) =>
          `flex flex-col items-center gap-1 text-[10px] relative ${isActive ? "text-white" : "text-zinc-400"}`
        }
      >
        <div className="relative">
          <Download size={24} />
    ...
    <div className="mt-3 flex flex-col md:flex-row min-h-screen bg-zinc-950 text-white relative">
      ...
      <main className="flex-1 overflow-y-auto pb-32 md:pb-36">
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8">
          <Outlet context={{ setIsCreateOpen }} />
        </div>
      </main>
