      ...
      return (
        <div className="flex flex-col gap-2">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="flex items-center gap-4 p-3 bg-zinc-900/20 rounded-lg"
            >
        ...
        {likedTracks.map((track) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h4
                className={`font-medium truncate ${currentTrack?.id === track.id ? "text-blue-400" : "text-white"}`}
              >
                {track.title}
              </h4>
    ...
    if (playlists.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <button
            onClick={() => navigate("/search")}
            className="mt-4 p-3 pl-8 pr-8 text-black font-bold bg-blue-500 rounded-full"
          >
            Find Music
          </button>
        </div>
            ...
            <div className="relative mb-4 aspect-square shadow-lg">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setOpenMenuId(
                    openMenuId === playlist.id ? null : playlist.id,
                  );
                }}
              ...
              {openMenuId === playlist.id && (
                ...
                >
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-2 text-xs text-zinc-300 hover:bg-zinc-800 transition-colors"
                  >
                    <Edit size={14} />
      ...
      <div className="flex items-center border-b border-zinc-800">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-1 px-4 py-3 text-xs font-bold transition ${
              activeTab === tab.id
